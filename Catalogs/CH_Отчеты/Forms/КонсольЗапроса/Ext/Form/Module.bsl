...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПараметрыЗапроса", МассивИменПараметров) Тогда
		ЗаполнитьИменаПараметров(МассивИменПараметров);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИменаПараметров(МассивИменПараметров)
	
	Для Каждого ИмяПараметра Из МассивИменПараметров Цикл
		
		НоваяСтрокаПараметр = ТаблицаПараметров.Добавить();
		НоваяСтрокаПараметр.Наименование = ИмяПараметра;
...
Процедура ПеренестиПараметрыЗапроса()
	...
	Для Каждого СтрокаПараметра Из ТаблицаПараметров Цикл
		...
		СтруктураПоиска = Новый Структура("Наименование", СтрокаПараметра.Наименование);
		...
		ЗаполнитьЗначенияСвойств(ВладелецФормы.Объект.ПараметрыЗапроса.Добавить(), СтрокаПараметра);
