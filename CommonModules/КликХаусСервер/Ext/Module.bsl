...
Функция ВыполнитьЗапросНаСервере(ТекстЗапроса, ИнформацияОВыполнении = Неопределено, СхемаКомпоновкиДанных = Неопределено) Экспорт 
	...
	Если Ответ.КодСостояния = 200 Тогда
		...
		Если Не СхемаКомпоновкиДанных = Неопределено Тогда
			Для Каждого Поле Из СхемаКомпоновкиДанных.НаборыДанных.ВнешДанные.Поля Цикл
				Если Строка(Поле.ТипЗначения) = "Дата" Тогда
					СтрокаДат = СтрокаДат + Поле.Поле + ",";	
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		ИнформацияОВыполнении = ПрочитатьJSON(Чтение,, СтрокаДат);
...
Функция ИнициализироватьСКД(Отчет_CH) Экспорт
	...
	СХ = Обработки.УниверсальныйОтчетClickHouse.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
...
Функция АктуализироватьСКД(СхемаКомпоновкиДанных, Отчет_CH) Экспорт 
	...
	Для Каждого Параметр Из Отчет_CH.ПараметрыЗапроса Цикл
		
		Если Не ПараметрыКомпоновки.Найти(Параметр.Наименование) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		НовыйПараметр = ПараметрыКомпоновки.Добавить();
		НовыйПараметр.Имя = Параметр.Наименование;
...
Функция СформироватьОтчет(ОтчетСсылка, НастройкиКомпоновки)
	...
	Результат = КликХаусСервер.ВыполнитьЗапросНаСервере(ТекстЗапроса, ИнформацияОВыполнении, СхемаКомпоновкиДанных);
	
	Если ИнформацияОВыполнении = Неопределено Тогда
		Возврат "Не удалось получить данные от ClickHouse" + Символы.ПС + Результат;
	КонецЕсли;
	
	ВнешДанные = ПреобразоватьИнформациюОВыполненииВТаблицуЗначений(ИнформацияОВыполнении, СхемаКомпоновкиДанных);	
